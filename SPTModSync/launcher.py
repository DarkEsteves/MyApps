...
DEFAULTS = {
    ...
    "directories": "user/mods;BepInEx/plugins",
}
...
class _Updater:
    ...
    @staticmethod
    def run(spt_host, spt_path, version_entry, installed):
        """Faz download do patch, apaga files_to_delete, extrai para spt_path."""
        host = _norm_host(spt_host)
        try:
            url = version_entry["Download"]
            if not url.lower().startswith("http"):
                url = f"http://{host}{url}"
            _emit("update_progress", {"msg": "A descarregar...", "pct": 5})
            ...
            with requests.get(url, stream=True, timeout=120) as r:
                ...
                with open(tmp_path, "wb") as f:
                    for chunk in r.iter_content(8192):
                        ...
                        if total:
                            _emit("update_progress", {"msg": "A descarregar...", "pct": 5 + int(done / total * 55)})
            # delete files_to_delete
            _emit("update_progress", {"msg": "A limpar obsoletos...", "pct": 62})
            for rel in version_entry.get("files_to_delete", []) or []:
                ...
                try:
                    ...
                    pass
            # extract
            _emit("update_progress", {"msg": "A extrair...", "pct": 70})
            with zipfile.ZipFile(tmp_path, "r") as zf:
                zf.extractall(spt_path)
            os.unlink(tmp_path)
            elapsed = max(0.01, time.time() - t0)
            size_mb = done / (1024 * 1024)
            speed = size_mb / elapsed
            _emit("update_progress", {"msg": "✔ Atualizado para " + version_entry.get("Version", ""), "pct": 100, "done": True})
            log_event("download_done", "OK", f"{size_mb:.1f}", f"{elapsed:.1f}", f"{speed:.2f}", host)
            return {"ok": True}
        except Exception as e:
            _emit("update_progress", {"msg": "❌ " + str(e), "pct": 0, "error": True})
            log_event("update_err", "ERR", str(e))
            return {"ok": False, "error": str(e)}
...
def _do_app_update(asset_url):
    ...
    try:
        ...
        with requests.get(asset_url, stream=True, headers={"User-Agent": APP_NAME}, timeout=120) as r:
            ...
            with open(tmp_path, "wb") as f:
                for chunk in r.iter_content(8192):
                    ...
                    if total:
                        _emit("app_update_progress", {"msg": "A descarregar...", "pct": int(done / total * 60)})
        _emit("app_update_progress", {"msg": "A extrair...", "pct": 80})
        ...
        with open(bat_path, "w") as bf:
            bf.write("\r\n".join(lines) + "\r\n")
        _emit("app_update_progress", {"msg": "✔ Update pronto! A app vai desligar. Liga-a manualmente para aplicar.", "pct": 100, "done": True, "shutdown": True})
...
def _api_do_update(self):
    ...
    if not res.get("update"):
        ...
        return res
    latest = res["latest"]
    log_event(f"A iniciar update para v{latest.get('Version', '')} <- {spt_host}", "INFO")
    threading.Thread(target=_Updater.run, args=(spt_host, spt_path, latest, installed), daemon=True).start()
    # grava a nova versão instalada após concluir
    def _finish():
        time.sleep(1)
